...
// allow places to be selected
var selectPlaceInteraction = new ol.interaction.Select({
  condition: ol.events.condition.singleClick,
  multi: false,
  layers: function (layer) {
    return layer.get('id') == 'placeLayer'
  }
})
// response when item is added to selection
selectPlaceInteraction.getFeatures().on('add', selectedFeatures)
// add to map
map.getInteractions().extend([selectPlaceInteraction])
...
function selectedFeatures (event) {
  var feature = event.target.item(0)
  var selected = placeName(feature.getProperties())
